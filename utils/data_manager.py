...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iImageNet1000, iCIFAR100_224, iImageNetR, iCUB200_224, iResisc45_224, \
                             iCARS196_224, iSketch345_224, vtab

from copy import deepcopy
import random


def custom_order(index):
    """
    Generate a list of integers based on the provided index.
    The list starts from index * 10 and loops back to the beginning after reaching 49.

    Args:
    - index (int): An integer in the range [0, 4] representing the starting point of the list.
    ...
    start = index * 10
    sequence = list(range(start, 50)) + list(range(0, start))
...
class FiveDsets_DataManager(object):
    ...
    def get_dataset(
        self, indices, source, mode, appendent=None, ret_data=False, m_rate=None,  with_raw=False, with_noise=False
    ):
        if source == "train":
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, dset_order=0):
        self.dataset_name = dataset_name
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        if shuffle:
            ...
            #order = idata.class_order
            order = custom_order(self.dset_order)
        self._class_order = order
        logging.info(self._class_order)
