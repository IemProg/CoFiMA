...
class Adapter(nn.Module):
    def __init__(self,
        ...
        self.down_proj = nn.Linear(self.n_embd, self.down_size, bias=True)
        self.non_linear_func = nn.ReLU()
        self.up_proj = nn.Linear(self.down_size, self.n_embd, bias=True)
    ...
    #TODO: add_residual should be Dynamic according to the argparser
    def forward(self, x, add_residual=True, residual=None, all_outputs=False):
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.config.ffn_adapt and self.config.ffn_option == 'parallel':
            adapt_x = self.adaptmlp(x, add_residual=self.config["adapter_residual"], all_outputs=False)
        ...
        if self.config.ffn_adapt:
            if self.config.ffn_option == 'sequential':
                x = self.adaptmlp(x, add_residual=self.config["adapter_residual"], all_outputs=False)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=0, embed_dim=768,
                 depth=12,
